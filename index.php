...
if (empty($_SESSION['_homepage_welcomed'])) {
    $_SESSION['_homepage_welcomed'] = true;
    $isFirstWelcome = true;
} else {
    $isFirstWelcome = false;
}

// ── Load events (same as calendar.php) ───────────────────────────────────
...
.sparkle:nth-child(1) { top:4px;  right:18px; animation-duration:2.1s; animation-delay:0s;    }
.sparkle:nth-child(2) { top:22px; right:8px;  animation-duration:2.8s; animation-delay:.6s;   }
.sparkle:nth-child(3) { top:6px;  right:48px; animation-duration:2.4s; animation-delay:1.1s;  font-size:10px; color:var(--purple-mid); }
.sparkle:nth-child(4) { top:32px; right:34px; animation-duration:3.2s; animation-delay:.3s;   font-size:9px;  color:var(--accent-blue); }
.sparkle:nth-child(5) { top:14px; right:70px; animation-duration:2.6s; animation-delay:1.7s;  font-size:11px; }
</style>
...
<div style="max-width:720px; margin:0 auto;">
  ...
  <div class="welcome-header">
    <div class="sparkle-wrap" aria-hidden="true">
    ...
    <h1 class="welcome-greeting">
      <?php if ($isFirstWelcome): ?>
        Welcome, <span class="welcome-name"><?= $firstName !== '' ? e($firstName) : 'friend' ?>!!</span>
      <?php else: ?>
        Welcome back, <span class="welcome-name"><?= $firstName !== '' ? e($firstName) : 'friend' ?>!!</span>
      <?php endif; ?>
    </h1>
    <p class="welcome-sub">Upcoming events across all your clubs.</p>
  </div>

  <!-- ── Events (identical to calendar.php) ──────────────────────────────── -->
  <?php if (empty($events)): ?>
    <div style="text-align:center; padding:60px 24px; color:var(--text-muted);">
      ...
      <a href="/clubs/browse.php" class="btn btn-primary">Browse Clubs</a>
    </div>
  <?php else: ?>
...
<!-- ── RSVP Change Modal (identical to calendar.php) ─────────────────────── -->
<div id="rsvp-modal"
