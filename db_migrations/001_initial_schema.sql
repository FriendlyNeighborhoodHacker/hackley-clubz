-- Migration 001: Initial schema for Hackley Clubz
-- This is the initial migration that sets up the complete database structure.
-- For a fresh installation, you can use schema.sql instead.
-- For an existing empty database, run this file.
...
INSERT IGNORE INTO settings (key_name, value) VALUES
  ...
  ('allowed_email_domains','students.hackleyschool.org,hackleyschool.org'),
...
CREATE INDEX IF NOT EXISTS idx_users_pwreset_expires    ON users(password_reset_expires_at);

-- Add FK from public_files back to users
ALTER TABLE public_files
  ADD CONSTRAINT IF NOT EXISTS fk_pf_created_by
...
CREATE INDEX IF NOT EXISTS idx_emails_sent_by_user_id ON emails_sent(sent_by_user_id);
CREATE INDEX IF NOT EXISTS idx_emails_sent_to_email   ON emails_sent(to_email);
CREATE INDEX IF NOT EXISTS idx_emails_sent_success    ON emails_sent(success);
...
CREATE INDEX IF NOT EXISTS idx_cm_club_id ON club_memberships(club_id);
CREATE INDEX IF NOT EXISTS idx_cm_user_id ON club_memberships(user_id);
